...
import { generateGradientStyle, getContrastTextStyles } from "@/lib/gradient-utils";
import { cn } from "@/lib/utils";
import { ChevronLeft, ChevronRight, Maximize, Minimize, Pause, Play, Volume2, X } from "lucide-react";
...
export function Slideshow({
	...
	const nextSlide = useCallback(() => {
		...
		if (hasMorePosts && onFetchMorePosts && !isFetchingMore) {
			const remainingPosts = posts.length - nextIndex;
			if (remainingPosts <= 3) {
	...
	}, [posts.length, isTransitioning, currentIndex, hasMorePosts, onFetchMorePosts, isFetchingMore]);
	...
	useEffect(() => {
		if (shouldFetchMore && onFetchMorePosts && hasMorePosts && !isFetchingMore) {
		...
		<div
			...
			style={
				backgroundColor
					? gradientStyle
					: {
							background: "linear-gradient(135deg, #fef7ed 0%, #ffffff 50%, #fef7ed 100%)",
					  }
			}
		>
			{progressBar()}
		...
		<>
			{images.slice(0, 6).map((image, index) => {
				...
				return (
					<div
						key={index}
						className={cn("absolute object-cover rounded-lg shadow-lg", position.size)}
						style={{
							top: position.top,
							left: position.left,
...
function VideoSlide({ videoUrl, textContent, creatorName, contrastTextStyles }: VideoSlideProps) {
