		...
		<div className="text-center py-20 relative">
			...
			<div className="relative">
				...
				>
					...
					<span
						className="font-bold"
						style={contrastTextStyles.accent}
					>
						{recipientName}
					</span>
					!
...
function PostCard({
	...
	useEffect(() => {
		const checkPermissions = () => {
			...
			setShowModerationControls((!!isModerator || !!isCreator) && userId !== post.creator?.id);
		};
	...
	}, [userId, post.creator?.id, post.createdAt, board.creatorId, isModerator, isCreator]);
	...
	return (
		<>
			...
			<Card
				...
				style={cardStyle}
				role="article"
				aria-label={`Post by ${post.isAnonymous ? post.anonymousName || "Anonymous" : post.creator.name || "Unknown"}`}
			>
				{media()}
...
export function BoardView({
	...
	const defaultClasses = getGradientClasses(backgroundColor, "fixed inset-0 w-full h-full");
	const textColors = getTextColors(backgroundColor);
	const contrastTextStyles = getContrastTextStyles(backgroundColor);
	...
	useEffect(() => {
		if (backgroundColor) {
			...
			document.body.style.background = "linear-gradient(135deg, #fef7ed 0%, #ffffff 50%, #fef7ed 100%)";
		}
